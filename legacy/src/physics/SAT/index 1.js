...
class SATResponse {
  ...
  static CalcSeparationVelocities(body1, body2, axis, contactPoint) {
    if (!body1.mass && !body2.mass) {
      return [new Vector(), 0, new Vector(), 0, 0]
    }
    ...
    return [vel1, ang1, vel2, ang2, j,ac1,ac2]
  }
  static ApplyFriction(body1, body2, axis, jn,ac1,ac2) {
    let a$va = new Vector(ac1.y * -body1.angVel.radian, ac1.x * body1.angVel.radian)
    let a$vb = new Vector(ac2.y * -body2.angVel.radian, ac2.x * body2.angVel.radian)
  ...
  static ApplyContactForce(body1, body2, gravity, axis) {
    if(body1.angularVelocity <1 && body1.velocity.magnitude() < 1){
      body1.velocity.set(0,0)
      body1.angularVelocity = 0
      //body1.acceleration.sub(gravity)
      
    }
       if(body2.angularVelocity <1 && body2.velocity.magnitude() < 1){
