...
function CollisionEventHandler(ev) {
  ev.data.body1.onCollision(ev.data, ev.data.body2)
  ev.data.body2.onCollision(ev.data, ev.data.body1[0])
}
class World {
  constructor() {
    this.objects = []
  ...
  narrowPhase() {
    for (var i = 0; i < this.contactList.length; i++) {
      ...
      this.eventDispatcher.emit("collision", manifold, CollisionEventHandler)
    }
  }
  broadPhase() {
    this.contactList = []
